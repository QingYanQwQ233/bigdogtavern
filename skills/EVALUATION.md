...
| `screen-flow-diagram` | 31 KB | **后追加**（用户确认"可以学习一下"）。把屏幕流转画成线框式导航图，是当前唯一能"把导航结构画出来"的手段，核对出口矩阵时有用 |
| `interactive-flow-diagram` | 26 KB | **后追加**。同上，抽象流程图版本（节点 + 动作标签） |
...
| `journey-map`（32 KB） | 面向「客户旅程 / 服务蓝图」，是 SaaS 产品的体验地图形态，与本项目（单机、单玩家、无客户）不对口 |
| `ux-map-maker` | 元技能，只在另外三个之间做选择；三个里已装两个，再加一层路由会与 `ui-design-system` 的入口职责重复 |

> **状态变更**：`screen-flow-diagram` 与 `interactive-flow-diagram` 初版被排除，经用户确认「可以学习一下」后**已改为安装**（见上面的已安装表）。
>
> **一处更正**：初版把排除理由写成「违反离线约束」，**这个理由不成立** —— 离线约束管的是随应用分发的资源；这些是开发侧产物（画给人看的图，浏览器打开即用），不进 `public/`、不进 APK，不受该约束管辖。判断依据应是相关性与形态。
>
> **CDN 依赖的处理**：这两个 skill 的产物从 `cdn.jsdelivr.net` 加载 D3 v7。**第三方内容保持原样不改**；离线场景下由使用者在**生成产物时**把那一行换成本地 D3（D3 是 MIT，可自行 vendor）—— 这属于使用方式，不属于修改 skill。
