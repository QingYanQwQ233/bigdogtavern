...
> **本文是 `docs/design/DESIGN_CONSTITUTION.md` 的「宿主契约」子文档。**
>
> 设计语言与总原则以**总纲**为准；本文只定义应用与世界卡之间的边界（8 区域 × 4 模式、主题 token、sandbox 能力），不重复定义总纲已有的内容。新增内容前先确认总纲里没有。
>
> 反向引用：总纲 §6（颜色）指向本文 §4，总纲 §10（无障碍）指向本文 §7，总纲 §13（定制）指向本文全文。
...
每条约束都标注了**验证方式**。没有标注的条款等于没有约束——这正是本节此前的问题。

| # | 约束 | 验证方式 |
|---|---|---|
| 7.1 | 移动端按钮命中区域至少 44px | `[静态]` `check_ui_accessibility.js` 守关键控件；`[运行时]` `audit_ui_accessibility.js` 测量全部可交互元素 |
| 7.2 | 输入框字体至少 16px，避免移动端自动缩放 | `[静态]` `check_ui_accessibility.js` 要求移动端 16px 覆盖存在；`[运行时]` 逐控件测量 computed font-size |
| 7.3 | 任何弹窗/抽屉都可用键盘关闭，焦点回到触发按钮 | `[静态]` 断言弹窗识别数 = Escape 分支覆盖数，且每个 close 函数都调用 `.focus()`；`[运行时]` 实测 Escape 关闭 + 焦点归还 |
| 7.4 | 必须保留 `:focus-visible` 焦点环 | `[静态]` 断言全局 `:focus-visible` 规则存在，且**每一处 `outline: none` 都有配对的焦点指示** |
| 7.5 | 尊重系统 `prefers-reduced-motion`，卡内 CSS 不得强制无限动画 | `[静态]` 断言 reduce 块数量且真正置 `transition/animation: none`；卡内脚本由 sandbox 授权链约束 |
| 7.6 | 长正文、长标题、空列表和错误状态都要有可读回退 | `[未强制]` 无客观判据，只能人工评审；已由「空列表/错误态」的实际渲染路径间接覆盖 |
| 7.7 | 状态不能只用颜色表达，图标或文字必须同步出现 | `[未强制]` 无法从 CSS 判定「是否有文字伴随」；依赖代码评审 |
| 7.8 | 卡内自定义消息区只能有一个滚动容器，禁止同时渲染重复叙事流 | `[静态]` 断言扩展 iframe 里 `[data-tavern-messages]` 是唯一消息区滚动容器，且叙事/选项容器不开新滚动容器 |

### 当前已知偏差（实测，待 UI 改版处理）

`audit_ui_accessibility.js` 在 390×844 视口下的实测结果：

- **7.1 命中区**：设置弹窗内 **27/29** 个控件高度小于 44px（表单输入 316×37、设置页签 58×33、`#btn-import`/`#btn-export` 74×28、复选框 16×16）。主界面（导航抽屉触发、发送、全屏输入、聊天输入框）已达标。
  导航项 `nav-item` 为 167×33~36 —— **这是刻意做窄的，接受为明确例外**。
- **7.2 输入框字号**：已修复。移动端表单控件统一 16px（此前设置弹窗内 14 个输入框/下拉为 13px）。
- **7.3 / 7.4 / 7.5 / 7.8**：实测通过。

改版时请把 7.1 的表单密度一并纳入；否则本表会长期挂着偏差记录。
