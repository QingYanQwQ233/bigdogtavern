...
聊天背景在「设置 → 界面」选择图片后通过 `/api/image-save` 持久保存；JSON 与 localStorage 只存路径和显示参数，不保存图片 base64，也不参与模型请求。Android 使用系统文件选择器，图片位于 `filesDir/nodejs/public/images/`，参数随 `settings.json` 恢复。导出的连接配置只包含图片路径，跨设备使用时需重新选择图片。关闭开关保留图片选择，移除恢复默认背景但不删除共享图片文件；加载失败回退主题背景。
