...
RPG 模式进入「世界库」：世界卡来自 JSON，可创建/打开彼此独立的世界存档。当前世界模式的开场、叙事时间线、RPG 状态、地图数据和 AI 美化图路径都由当前 `WorldSave` 持有；旧 RPG `sessions` 保留兼容路径，不会被世界模式读取或改写。
...
- **世界存档边界**：世界模式从 `WorldSave.turns` 投影叙事，状态 / 背包 / 任务 / 地图 / 图片只随 `saveId` 切换；服务端用 `expectedRevision` 顺序写回，旧 RPG 会话仍以 `session.rpgState` 持久化且暂不互相迁移
...
- [ ] W4 世界角色静态定义与存档动态状态（已完成玩家快照 / `npcStates` / 稳定 `locationId` / NPC Prompt 作用域 / 秘密解锁边界 / 存档内生成实体 / 显式收录 NPC 创建不可变世界版本；队伍 UI 待补）
