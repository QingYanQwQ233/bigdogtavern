...
assert.match(appJs, /html: rendered\.html/);
assert.match(appJs, /narrativeHtml: narrativeRendered\.html/);
assert.match(appJs, /const syncMessageSurface = \(\) =>/);
assert.match(appJs, /target\.dataset\.tavernAllowDuplicate/);
assert.match(appJs, /function worldCardUsesCustomLayout\(world = currentWorldCard\(\)\)/);
assert.match(appJs, /world-custom-layout/);
assert.match(appJs, /setWorldCustomLayout\(false\);\s*clearWorldExtension\(\)/);
assert.match(appJs, /if \(mode === 'rpg' && worldModeActive\(\)\) \{\s*closeWorldLibrary\(\);\s*enterWorldWorkspace\(\)/);
assert.match(fs.readFileSync(path.join(root, 'public', 'styles.css'), 'utf8'), /world-custom-layout:not\(\.world-immersive\) \.sidebar/);
assert.match(fs.readFileSync(path.join(root, 'public', 'styles.css'), 'utf8'), /world-custom-layout:not\(\.world-immersive\) \.chat-header/);
...
assert.match(appJs, /on: \(name, listener\)/);
assert.match(appJs, /agent\.complete/);
assert.match(appJs, /turn\.commit/);
...
assert.doesNotMatch(indexHtml, /world-immersive-exit/);
assert.match(appJs, /layout === 'custom'/);
assert.match(appJs, /e\.key !== 'Escape'/);
assert.match(appJs, /type: 'immersive\.exit'/);
assert.match(appJs, /if \(data\.type === 'immersive\.exit'\)/);
assert.match(appJs, /fullscreen: \(\) => send\('immersive\.fullscreen'\)/);
assert.match(appJs, /exitFullscreen: \(\) => send\('immersive\.exit'\)/);
assert.match(appJs, /if \(data\.type === 'immersive\.fullscreen'\)/);
assert.match(appJs, /#tavern-extension-root>:first-child\{box-sizing:border-box;min-height:100%/);
